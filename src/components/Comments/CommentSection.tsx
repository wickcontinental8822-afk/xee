...
import { CommentDisplay } from './CommentActions';
...
export function CommentSection({ stageId }: CommentSectionProps) {
  const { user } = useAuth();
  const { commentTasks, addCommentTask, stages, updateCommentTaskStatus, updateCommentTask, deleteCommentTask } = useData();
        ...
        {stageComments.map(comment => (
          <div key={comment.id} className={`border rounded-lg p-4 ${
            ...
            <div className="text-gray-700 mb-3">
              <CommentDisplay
                comment={comment}
                onUpdate={updateCommentTask}
                onDelete={deleteCommentTask}
              />
            </div>
